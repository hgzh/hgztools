	...
	if (isset($par_lang) && $par_lang != '' && isset($par_project) && $par_project != '' && isset($par_page) && $par_page != '') {
		...
		if( $q1->num_rows === 0 ) {
			...
			$page->addInline('p', 'found ' . $q1->num_rows . ' revisions for article ' . 
				'<a href="https://' . $par_lang . '.' . $par_project . '.org/wiki/' . $par_page . '">' . $par_page . '</a> (<a href="https://' . $par_lang . '.' . $par_project . '.org/w/index.php?title=' . $par_page . '&action=history">History</a>).');
